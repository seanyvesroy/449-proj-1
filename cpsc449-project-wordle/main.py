...
async def loginUser():
    ...
    if data:
     try:
      ...
      values={"user": data['username'], "pass": data['password']}
      userDet = await db.fetch_one("select * from USERDATA where user_name = :user and user_pass= :pass",values,)
      app.logger.info("""select * from USERDATA where user_name = :user and user_pass= :pass""",values,)
      if (userDet is None): 
...
async def startGame():
    ...
    userCheck = await db.fetch_one("select user_id from USERDATA where user_id = :user_id",values,)
    app.logger.info("select user_id from USERDATA where user_id = :user_id",values,)
    if userCheck == None:
...
async def gamestate():
    ...
    gamestate = await db.fetch_all("select * from USERGAMEDATA where game_id = :game_id", values,)
    app.logger.info("select * from USERGAMEDATA where game_id = :game_id", values,)
    
    guesses = await db.fetch_all("select guess_num, guessed_word from guess where game_id = :game_id", values,)
    app.logger.info("select guess_num, guessed_word from guess where game_id = :game_id", values,)
...
async def make_guess():
    ...
    try:
        ...
        app.logger.info("""SELECT word_id FROM VALIDWORD WHERE EXISTS(SELECT word_id FROM VALIDWORD WHERE valid_word = :guess)""", string_guess,)
    except sqlite3.IntegrityError as e:
        abort(500, e)
